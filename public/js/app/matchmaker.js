...
window.App = function() {
  ...
  _.personModel = function(data) {
    ...
    o.photos = ko.observable(
      $.map(data.photos, function(item) {
        return new _.photoModel(item);
      })
    );
    o.bio = ko.observable(data.bio);
    o.mainPhoto = ko.observable(
      data.photos.length != 0
        ? data.photos[0].processedFiles[3].url
        : "/assets/img/user-generic.png"
    );
    o.ping_time = ko.observable(data.ping_time);
    o.distance_mi = ko.observable(
      typeof data.distance_mi == "undefined" ? null : data.distance_mi
    );
  ...
  _.dislike = function(data, event) {
    $(".loader-global").show();
    $.getJSON("/t/" + getAuthToken() + "/dislike/" + data._id(), function(
      result
    ) {
      _.recs.remove(data);
      $(".loader-global").hide();
